...
- E = computeEnergy(neighborhoodWeights, double(L==1), objProbabilitees, bkgProbabilitees); 
	...
	- with this, we can find if the probabilities of the background has changed from the last iteration
	- this is number 5
- Number 6
	- we want user inputs (background, foreground)
	- the user will have to draw on the image itself (can use a rectangle or pencil or tagging or whatever)
- 6e, for objProbabilitees, bkgProbabilitees
